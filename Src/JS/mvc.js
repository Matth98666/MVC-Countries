...
window.onload = () => {
    ...
    let titlesColor = Cookies.get('titlesColor');
    let textsColor = Cookies.get('textColor');
    let encartsColor = Cookies.get('encartsColor');

    if (backgroundColor !== undefined) {
        d.body.style.backgroundColor = backgroundColor;
    }

    if (titlesColor !== undefined) {
        d.querySelectorAll('h1,h2,h3,h4,h5,h6').forEach(title => {
            title.style.color = titlesColor;
        });
    }

    if (textsColor !== undefined) {
        d.querySelectorAll('p,td').forEach(text => {
            text.style.color = textsColor;
        });
    }

    if (encartsColor !== undefined) {
        d.querySelectorAll('.text-bg-primary, .bg-primary, .active').forEach(encart => {
            encart.style.setProperty('background' , $this.value , "important");
        });
    }
    ...
    d.querySelector('#mvcOffCanvas').addEventListener('input', (e) => {
        ...
        switch ($this.id) {
            ...
            case 'titles-color':
                ...
                Cookies.set('titles-color', $this.value);
            ...
            case 'texts-color':
                ...
                Cookies.set('texts-color', $this.value);
                break;
        }
    }, true);
